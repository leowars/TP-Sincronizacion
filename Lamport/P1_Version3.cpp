...
int main() {
    ...
    std::cout << "Proceso P1 corriendo en puerto " << my_port << std::endl;

    increment_clock();
    std::string msg = serialize(lamport_clock, id, "request");
    std::cout << "P1 inicia y envia a P2" << std::endl;
    send_message(next_ip, next_port, msg);

    // Solo recibe y ejecuta una vez
    std::string recv_msg;
    std::cout << "P1 esperando mensaje (por si quieres hacer un ciclo)..." << std::endl;
    ...
    std::cout << "P1 recibe mensaje de P" << m_id << " con clock " << m_clock << std::endl;
    std::cout << "P1 ejecutando su accion. Clock local: " << lamport_clock << std::endl;

    std::this_thread::sleep_for(std::chrono::seconds(1));
    std::cout << "P1 finaliza." << std::endl;
    return 0;
}