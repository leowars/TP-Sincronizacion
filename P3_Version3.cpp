...
int main() {
    ...
    std::cout << "Proceso P3 corriendo en puerto " << my_port << std::endl;

    std::string recv_msg;
    std::cout << "P3 esperando mensaje..." << std::endl;
    ...
    std::cout << "P3 recibe mensaje de P" << m_id << " con clock " << m_clock << std::endl;
    std::cout << "P3 ejecutando su accion. Clock local: " << lamport_clock << std::endl;

    std::this_thread::sleep_for(std::chrono::seconds(1));

    increment_clock();
    std::string msg = serialize(lamport_clock, id, "request");
    std::cout << "P3 envia mensaje a P4" << std::endl;
    send_message(next_ip, next_port, msg);

    std::cout << "P3 finaliza." << std::endl;
    return 0;
}